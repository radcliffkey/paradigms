...
public class WeightedLevenDist implements StringDistanceMetric {
	
	private SymmetricSparseMatrix distMatrix;
	
	public WeightedLevenDist(SymmetricSparseMatrix distMatrix) {
		super();
		...
		for (int i = 0; i < l1; ++i) {
			...
			for (int j = 1; j < curr.length; ++j) {
				if (s1.charAt(i) == s2.charAt(j - 1)) {
					...
					double substDist =  prev[j - 1] + this.distMatrix.get(s1.charAt(i), s2.charAt(j - 1));
