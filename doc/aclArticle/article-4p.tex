...
%Kohonen used seeds to improve the results of Morfessor (), however their approach uses significantly larger seeds and they aim to improve morpheme-segmentation algorithm. 
%Kohonen
...
%Thus, a scheme is uniquely defined by its c-suffix set.
...
%From every pair of stem allomorphs in the seed, $a\delta_1, a\delta_2$, where $a$ is their longest common initial substring,\footnote{should $\delta_1$ or $\delta_2$ be 0, one final character is removed from $a$ and prepended to $\delta_1$ and $\delta_2$} with suffix
%sets $F_1$, $F_2$ we generate a rule $*\delta_1 \rightarrow *\delta_2$ / $(F_1, F_2)$ and also a reverse rule $*\delta_2 \rightarrow *\delta_1$ / $(F_2, F_1)$. A rule $*\delta_1 \rightarrow *\delta_2$ / $(F_1, F_2)$ means that any stem $x\delta_1$ can be transformed into $x\delta_2$ if $x\delta_2$ is a c-stem present in the corpus and:
%
%\begin{enumerate}
...
%
%\noindent
%Induced rules are applied to generate so-called \emph{deep} stems from the c-stems. We define relation $\leftrightarrow$ between two c-stems: $s_1 \leftrightarrow s_2$ iff there is a rule transforming $s_1$ to $s_2$ and a rule transforming $s_2$ to $s_1$. We construct an equivalence $\Leftrightarrow$ as a transitive and reflexive closure of $\leftrightarrow$. The deep stems then correspond to equiv. classes of $\Leftrightarrow$.
\noindent
Formally, the process can be described as follows.
From every pair of stem allomorphs in the seed, $a\delta_1, a\delta_2$, where $a$ is their longest common initial substring,
%\footnote{should $\delta_1$ or $\delta_2$ be 0, one final character is removed from $a$ and prepended to $\delta_1$ and $\delta_2$} 
with suffix
...
\noindent
%These rules were used to generate underlying form of c-stems, which we call \emph{deep} stems.  We define relation $\leftrightarrow$ between two c-stems: $s_1 \leftrightarrow s_2$ iff there is a correspondence between $s_1$ and $s_2$ licensed by any rule. We construct an equivalence $\Leftrightarrow$ as a transitive and reflexive closure of $\leftrightarrow$. The deep stems then correspond to equiv. classes of $\Leftrightarrow$.
...
%
%These rules were used to generate underlying form of c-stems, which we call \emph{deep} stems. Two c-stems are related by $\leftrightarrow$ iff there is a correspondence between them licensed by any rule. The deep stems then correspond to equivalence classes induced by the reflexive and transitive closure of $\leftrightarrow$.
%
%
...
We tested our approach on Czech, Slovene, German and Catalan lemmatised corpora, summarised in Table \ref{table:corpora}. %For Czech, we used two differently sized subsets of the PDT 1 corpus. The first, marked as \textbf{cz1}, contains 11k types belonging to 6k lemmas. The second, \textbf{cz2}, has 27k types and 13k lemmas. The Slovene corpus \textbf{si} is a subset of the jos100k corpus V2.0\footnote{\url{http://nl.ijs.si/jos/jos100k-en.html}} with 27k types and 15.5k lemmas. The corpus of German is a subset of the TIGER corpus\footnote{\url{http://www.ims.uni-stuttgart.de/projekte/TIGER/}} containing 22k types and 17k lemmas. The TIGER corpus consists of German news text.
...
%cz1 & PDT1 & 11k & 6k & 10k & 5.5k\\\hline
%cz2 & PDT1 & 27k & 13k & 25k & 12k\\\hline
...
\end{table}
 \footnotetext[2]{\url{http://ufal.mff.cuni.cz/pdt/}} 
 \footnotetext[3]{\url{http://nl.ijs.si/jos/jos100k-en.html}} \footnotetext[4]{\url{http://www.ims.uni-stuttgart.de/projekte/TIGER}}


\subsection{Evaluation method}

\noindent
We evaluated the experiments only on types at least 4 characters long to avoid most of the closed-class and irregular words. %Sizes of corpora in word types / lemmas after filtering were following: 10k/5.4k (\textbf{cz1}), 25k/12k (\textbf{cz2}), 25k/14k (\textbf{si}), 21k/16k (\textbf{de}), 10k/7k (\textbf{si})
...
%\item \label{exp:manseed} \e{seed} -- seed was used
%\item \e{seed + pref.} -- seed was used together with additional rules for two Czech inflectional prefixes, otherwise same as \ref{exp:manseed}.
%\end{enumerate}
...
%\hline
%\bf Corpus & \bf Experiment & \bf Precision & \bf Recall & \bf F1\\
%\hline
%cz1&no seed &  90.61 & 47.51 & 62.33 \\
%   &seed    & 90.14 &  53.67 & 67.28 \\
%   &seed + pref. & 88.18 & 61.23 & \bf 72.28\\ \hline
%cz2&no seed & 87.65 & 57.75 & 69.63\\
%   &seed    & 87.14 & 62.43 & 72.74\\
%   &seed + pref. & 86.31 & 71.59 & \bf 78.26\bf \\ \hline
%si &no seed & 69.98 & 80.40 & 74.83\\
%   &seed    & 69.60 & 82.74 & \bf 75.61 \\ \hline
%de &no seed & 56.02 & 64.87 & 60.12 \\
%   &seed    & 54.67 & 66.69 & 60.08 \\ \hline
%ca &no seed & 57.71 & 68.72 & 62.74\\
%   &seed    & 60.84 & 71.99 & \bf 65.95\\
%\hline
%\end{tabular}
%\end{center}
%\end{small}
%\caption{\label{table:res} Results for the for various corpora.}
%\end{table}
...
\hline
%cz1&no seed      &  0 &  90.61 & 47.51 & 62.33 \\
%   &seed         & 18 & 90.14 &  53.67 & 67.28 \\
%   &seed + pref. & 18 & 88.18 & 61.23 & \bf 72.28\\ \hline
...
%   &seed         & 11 & 54.67 & 66.69 & 60.08 \\ \hline
   &seed         & 11 & 54.67 & 72.69 & 63.40 \\ \hline  % corrected seed, waiting for results, non-direct eval suggest s something like this
...
%However, it did not help in German, inspecting the results suggests several reasons: (1) most paradigms have very small numbers of forms, (2) frequent root-vowel changes, (3) many composites (where the last part is often mistaken for an affix).
